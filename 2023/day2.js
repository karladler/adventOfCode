...
zoneight234
7pqrstsixteen`;
const input2 = `six1mpffbnbnnlxthree
4eight3one92
9nine2xnhvjtjlzj48
...
const r = input.split('\n').reduce((s, l, idx) => {
  ...
  const up = l.replace(new RegExp(w.join('|'), 'g'), (m) => w.indexOf(m)+1);
  const d = up.replace(w).match(/\d/g);
  if(idx < 3) {
    console.log(l)
    console.log(up)
    console.log(d);
    console.log(parseInt(d.at(0) + d.at(-1)))
  }
  return s += parseInt(d.at(0) + d.at(-1))
}, 0)
