...
const res1 = inputs.filter((inp => {
    const [min, max, char, password] = inp as unknown as any[];
    let count = [...password].filter((c: string) => c === char).length;
    return count >= min && count <= max;
}))

console.log('res1', res1.length);

const res2 = inputs.filter((inp => {
  const [pos1, pos2, char, password] = inp as unknown as any[];
